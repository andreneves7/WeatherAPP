...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val m = intent.getStringExtra("marca")
        Log.d("Mapa", "m  ${m}")
        if (m != null) {
            marca = m.toInt()
        }
        ...
        if (marca == 1 ){
            ...
            Log.d("Mapa", "latitude2 ${latitude}}")
            Log.d("Mapa", "longitude2 ${longitude}}")

            weatherLatLog().execute()
        }
    ...
    inner class weatherLatLog() : AsyncTask<String, Void, String>() {
        ...
        override fun doInBackground(vararg params: String?): String? {
            ...
            try {
                response =
                    URL("https://api.openweathermap.org/data/2.5/weather?lat=$Lat&lon=$Long&appid=1b5ce9dd86efb73dab71ebd469cce738").readText(
                        Charsets.UTF_8
                    )
            } catch (e: Exception) {
                response = null
